...
| --- | --- | --- |
| `provider` | ✅ | The vault scheme, e.g. `aws`, `databricks`. Must be a lowercase name. Selects which registered [provider](providers.md) resolves the secret. |
| `region` | — | Provider-specific location or profile. May be **empty**. For AWS it is the region; for Databricks it is the CLI profile name. |
| `secret_name` | ✅ | The secret's identifier within the vault (an AWS secret name, a Databricks scope, …). |
| `secret_key` | ✅ | The key selected from within the secret (a JSON key for AWS, a secret key for Databricks). |

Only `region` may be empty. `provider`, `secret_name`, and `secret_key` must all be non-empty.
...
- `aws:us-east-1:my-secret:OPENAI_API_KEY` — the `OPENAI_API_KEY` key of the JSON secret `my-secret`
  in region `us-east-1`.
- `databricks::my-secret-scope:OPENAI_API_KEY` — an **empty** region means the default Databricks
  workspace/profile.
- `databricks:staging:my-secret-scope:MY_KEY` — a non-empty region names the Databricks CLI
  profile `staging`.
...
- `region` matches `^[A-Za-z0-9._-]*$` — letters, digits, `.`, `_`, `-`, or empty.
- `secret_name` and `secret_key` are non-empty.

The strict `region` grammar is deliberate: it prevents colon-heavy strings such as
...
An **empty region** is normalized to `None` on the parsed
[`SecretProperties`](api/sri.md#vaultriever.sri.SecretProperties):

```python
parse_sri('databricks::my-scope:MY_KEY').region  # None
```
